...
#define LIL_CHECKARGS(lil, cmd, argc, lo, hi) do { if (argc < lo || argc > hi) { LIL_FAILED(lil, "Expected %i-%i args to " cmd ", got %i", lo, hi, argc); return NULL; } } while (0)

#define LIL_NOARGS(lil, cmd, argc) do { if (argc != 0) { LIL_FAILED(lil, cmd " takes no arguments (got %i)", argc); return NULL; } } while (0)
...
#define LIL_ENDSUBCOMMANDS(lil, topcmd, argv) LIL_FAILED(lil, "invalid subcommand %s for command " topcmd, lil_to_string(argv[0]));
